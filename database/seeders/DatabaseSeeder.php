    ...
    public function run(): void
    {
        ...
        foreach ($students as $row) {
            $user = User::create([
                ...
                'status' => $row['status'],
                'phone_verified_at' => $row['verified'] ? now() : null,
            ]);
        ...
        foreach ($yearData as $index => $yearDataRow) {
            $years[] = AcademicYear::create([
                'title' => ['ar' => $yearDataRow[0], 'en' => $yearDataRow[1]],
                'subtitle' => ['ar' => $yearDataRow[2], 'en' => $yearDataRow[3]],
                'icon' => $yearDataRow[4],
                'sort_order' => $index + 1,
                'is_active' => true,
            ]);
        ...
        foreach (array_slice($subjects, 0, 12) as $index => $subject) {
            $definition = $courseDefinitions[$index % count($courseDefinitions)];
            $slug = Str::slug($definition[1]).'-'.$subject->id;

            $course = Course::create([
                'subject_id' => $subject->id,
                'slug' => $slug,
                'title' => ['ar' => $definition[0], 'en' => $definition[1]],
                'short_description' => ['ar' => $definition[2], 'en' => $definition[3]],
                'description' => ['ar' => $definition[2], 'en' => $definition[3]],
                'status' => 'published',
                'is_featured' => $definition[4],
                'published_at' => now()->subDays(30 - $index),
            ]);

            $courses[] = $course;

            for ($videoNumber = 1; $videoNumber <= 4; $videoNumber++) {
                CourseVideo::create([
                    'course_id' => $course->id,
                    'title' => [
                        'ar' => "المحاضرة {$videoNumber}: {$definition[0]}",
                        'en' => "Lecture {$videoNumber}: {$definition[1]}",
                    ],
                    'lesson_label' => ['ar' => "الدرس {$videoNumber}", 'en' => "Lesson {$videoNumber}"],
                    'source_path' => "courses/{$course->id}/videos/lecture-{$videoNumber}.mp4",
                    'hls_manifest_path' => "courses/{$course->id}/hls/lecture-{$videoNumber}/master.m3u8",
                    'duration_seconds' => 2400 + ($videoNumber * 300),
                    'sort_order' => $videoNumber,
                    'is_preview' => $videoNumber === 1,
                    'is_downloadable' => $videoNumber > 1,
                    'status' => 'ready',
                ]);
            }
            ...
            for ($fileNumber = 1; $fileNumber <= 2; $fileNumber++) {
                CourseFile::create([
                    'course_id' => $course->id,
                    'title' => ['ar' => "ملف المراجعة {$fileNumber}", 'en' => "Review File {$fileNumber}"],
                    'file_path' => "courses/{$course->id}/files/review-{$fileNumber}.pdf",
                    'original_name' => "course-{$course->id}-review-{$fileNumber}.pdf",
                    'mime_type' => 'application/pdf',
                    'extension' => 'pdf',
                    'size_bytes' => (2 + $fileNumber) * 1024 * 1024,
        ...
        $firstVideo = $courses[0]->videos()->firstOrFail();

        VideoProgress::create([
            'user_id' => $users[0]->id,
            'course_video_id' => $firstVideo->id,
        ...
        foreach ($codes as $code) {
            SubscriptionQrCode::create([
                'course_id' => $code[1]->id,
                'code_hash' => hash('sha256', $code[0]),
                'code_hint' => Str::mask($code[0], '*', 6, -4),
                'label' => $code[0],
                'starts_at' => now()->subDay(),
                'expires_at' => now()->addYear(),
                'max_redemptions' => $code[3],
                'redemptions_count' => 0,
                'subscription_duration_days' => $code[2],
                'status' => 'active',
        ...
        foreach ($pages as $page) {
            ContentPage::create([
                'slug' => $page[0],
                'title' => ['ar' => $page[1], 'en' => $page[2]],
                'content' => ['ar' => $page[3], 'en' => $page[4]],
                'is_active' => true,
            ]);
        }
    }
