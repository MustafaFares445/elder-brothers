...
class SimplifiedCourseRulesTest extends TestCase
{
    ...
    public function test_course_slug_is_generated_and_media_rules_are_forced(): void
    {
        ...
        $video = CourseVideo::query()->create([
            ...
            'source_path' => 'courses/1/videos/video.mp4',
        ...
        $file = CourseFile::query()->create([
            'course_id' => $course->id,
            'title' => ['ar' => 'ملف الدورة'],
            'external_url' => 'https://example.com/course.pdf',
    ...
    public function test_qr_code_is_encrypted_single_use_and_valid_for_two_days(): void
    {
        $year = AcademicYear::query()->create([
            'title' => ['ar' => 'السنة الأولى'],
            'sort_order' => 1,
        ...
        $this->assertTrue(
            $qrCode->fresh()->expires_at->between(
