<?php

return [
    'operation_completed' => 'تمت العملية بنجاح.',
    'validation_failed' => 'البيانات المرسلة غير صالحة.',
    'server_error' => 'حدث خطأ غير متوقع في الخادم.',
    'request_failed' => 'تعذر إكمال الطلب.',
    'account_inactive' => 'الحساب غير مفعل بعد. يرجى انتظار تفعيله من الإدارة.',
    'account_pending_activation' => 'تم إنشاء الحساب بنجاح وهو بانتظار تفعيله من الإدارة.',
    'account_suspended' => 'تم إيقاف هذا الحساب.',
    'registration_disabled' => 'التسجيل الجديد متوقف مؤقتاً.',
    'phone_already_registered' => 'رقم الهاتف مسجل مسبقاً.',
    'verification_sent' => 'تم إرسال رمز التحقق بنجاح.',
    'phone_verified' => 'تم التحقق من رقم الهاتف بنجاح.',
    'otp_resend_not_available' => 'يرجى الانتظار قبل طلب رمز جديد.',
    'otp_invalid' => 'رمز التحقق غير صحيح.',
    'otp_expired' => 'انتهت صلاحية رمز التحقق.',
    'otp_attempts_exceeded' => 'تم تجاوز عدد محاولات التحقق المسموح.',
    'invalid_credentials' => 'رقم الهاتف أو كلمة المرور غير صحيحة.',
    'phone_not_verified' => 'لم يتم التحقق من رقم الهاتف.',
    'logged_in' => 'تم تسجيل الدخول بنجاح.',
    'logged_out' => 'تم تسجيل الخروج بنجاح.',
    'otp_verified' => 'تم قبول رمز التحقق.',
    'reset_token_invalid' => 'رمز استعادة كلمة المرور غير صالح أو منتهي.',
    'password_reset' => 'تم تغيير كلمة المرور بنجاح.',
    'profile_updated' => 'تم تحديث الملف الشخصي بنجاح.',
    'avatar_updated' => 'تم تحديث الصورة الشخصية بنجاح.',
    'current_password_incorrect' => 'كلمة المرور الحالية غير صحيحة.',
    'password_updated' => 'تم تحديث كلمة المرور بنجاح.',
    'preferences_updated' => 'تم تحديث التفضيلات بنجاح.',
    'device_registered' => 'تم تسجيل الجهاز بنجاح.',
    'device_removed' => 'تم حذف الجهاز بنجاح.',
    'qr_invalid' => 'رمز QR غير صالح.',
    'qr_valid' => 'رمز QR صالح.',
    'qr_disabled' => 'رمز QR معطل.',
    'qr_expired' => 'انتهت صلاحية رمز QR.',
    'qr_not_started' => 'رمز QR غير فعال بعد.',
    'qr_limit_reached' => 'تم الوصول إلى الحد الأقصى لاستخدام رمز QR.',
    'qr_already_used' => 'لقد استخدمت رمز QR هذا مسبقاً.',
    'subscription_activated' => 'تم تفعيل اشتراك الدورة بنجاح.',
    'playback_authorized' => 'تم السماح بتشغيل الفيديو.',
    'progress_saved' => 'تم حفظ تقدم الفيديو.',
    'video_completed' => 'تم إكمال الفيديو.',
    'download_authorized' => 'تم السماح بالتنزيل.',
    'notification_read' => 'تم تحديد الإشعار كمقروء.',
    'notifications_read' => 'تم تحديد الإشعارات كمقروءة.',
    'support_created' => 'تم إنشاء طلب الدعم بنجاح.',
];